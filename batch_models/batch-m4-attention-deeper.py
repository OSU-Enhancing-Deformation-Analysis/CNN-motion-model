...
class AttentionDeeperWiderMotionVectorRegressionNetwork(DeeperWiderMotionVectorRegressionNetwork):  # Inherit DeeperWider for structure
    def __init__(self, input_images=2, base_channels=64, num_conv_blocks=5):
        ...
        self.attention = SelfAttention(channels)  # Attention at deepest point, adjust channels

        # Modify output path to match attention channel count
        self.output = nn.Sequential(
            # Upsampling path - adjusted to match deeper convolution
            nn.ConvTranspose2d(channels, channels // 2, kernel_size=4, stride=2, padding=1),  # Upsample to 64x64
            nn.LeakyReLU(0.1),
            nn.BatchNorm2d(channels // 2),
            nn.ConvTranspose2d(channels // 2, channels // 4, kernel_size=4, stride=2, padding=1),  # Upsample to 128x128
    ...
    def forward(self, x):
        x = self.convolution(x)  # Use the deeper/wider convolution
        x = self.attention(x)  # Apply attention
        x = self.output(x)  # Use the original output path
        return x


model = AttentionDeeperWiderMotionVectorRegressionNetwork(input_images=2).to(device)
print(model)
...
wandb_config = {
    ...
    "architecture": "AttentionDeeperWiderMotionVectorRegressionNetwork",
    "dataset": {
