...
class AttentionMotionVectorRegressionNetwork(nn.Module):
    def __init__(self, input_images=2):
        ...
        self.convolution = nn.Sequential(
            ConvolutionBlock(input_images, 32, kernel_size=3),
            nn.MaxPool2d(kernel_size=2),
            ConvolutionBlock(32, 64, kernel_size=3),
            nn.MaxPool2d(kernel_size=2),
            ConvolutionBlock(64, 128, kernel_size=3),
            ConvolutionBlock(128, 128, kernel_size=3),
            SelfAttention(128),  # Self-Attention layer added here
        )
        ...
        self.output = nn.Sequential(
            ...
            nn.LeakyReLU(0.1),
            nn.BatchNorm2d(32),
            nn.Conv2d(32, self.vector_channels, kernel_size=3, stride=1, padding=1),
        )
...
model = AttentionMotionVectorRegressionNetwork(input_images=2).to(device)
print(model)
...
wandb_config = {
    ...
    "architecture": "AttentionMotionVectorRegressionNetwork",
    "dataset": {
