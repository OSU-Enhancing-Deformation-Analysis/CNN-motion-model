...
#  This model has changes to data loading a more patterns for variation.
#  A few patterns have been removed because they cause discontinutiies.
#  
#  Masks will morph and change the vector field to add more sharpness to the data.
#  There is a blur option, it is disabled in this file, but there is a modified
#  one with it enabled.
#  
...
class UNetAttentionMotionVectorRegressionNetwork(nn.Module):
    def __init__(self, input_images=2):
        ...
        # Downsampling path
        self.conv1 = ConvolutionBlock(input_images, 32)
        self.pool1 = nn.MaxPool2d(kernel_size=2)
        self.conv2 = ConvolutionBlock(32, 64)
        self.pool2 = nn.MaxPool2d(kernel_size=2)
        self.conv3 = ConvolutionBlock(64, 128)
        self.conv4 = ConvolutionBlock(128, 128)
        self.attention = SelfAttention(128)  # Attention in bottleneck

        # Upsampling path
        self.upconv1 = nn.ConvTranspose2d(128, 64, kernel_size=4, stride=2, padding=1)
        self.conv_up1 = ConvolutionBlock(128, 64)  # Input channels are doubled due to skip connection
        self.upconv2 = nn.ConvTranspose2d(64, 32, kernel_size=4, stride=2, padding=1)
        self.conv_up2 = ConvolutionBlock(64, 32)  # Input channels are doubled due to skip connection

        self.output_conv = nn.Conv2d(32, self.vector_channels, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        attention_output = self.attention(conv4)  # Apply attention

        # Upsampling
        upconv1 = self.upconv1(attention_output)
        upconv1_concat = torch.cat([upconv1, conv2], dim=1)  # Skip connection from conv2
        conv_up1 = self.conv_up1(upconv1_concat)

        upconv2 = self.upconv2(conv_up1)
        upconv2_concat = torch.cat([upconv2, conv1], dim=1)  # Skip connection from conv1
        conv_up2 = self.conv_up2(upconv2_concat)

        output = self.output_conv(conv_up2)
        return output


model = UNetAttentionMotionVectorRegressionNetwork(input_images=2).to(device)
print(model)
...
wandb_config = {
    ...
    "architecture": "UNetAttentionMotionVectorRegressionNetwork",
    "dataset": {
