    ...
    if epoch % SAVE_FREQUENCY == 0:
        with torch.no_grad():
            ...
            for i, images in enumerate(samples_images):
                ...
                converted_pred = (
                    sample_predictions[i][0].cpu().numpy().transpose(1, 2, 0)
                )
                converted_pred = flow_to_image(converted_pred)
            ...
            for tile in tiles:
                ...
                with torch.no_grad():
                    ...
                    converted_pred = pred[0][0].cpu().numpy().transpose(1, 2, 0)
                    converted_pred = flow_to_image(converted_pred)
                    # converted_pred = np.vstack(
                    #     (
                    #         converted_pred,
                    ...
                    combined = np.hstack(
                        (base_image, next_time, next2_time, converted_pred)
                    ).astype(np.uint8)
