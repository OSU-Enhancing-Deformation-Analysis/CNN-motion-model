...
from typing import Dict, Callable, Tuple, List, TypeAlias
import numpy as np
...
# --- Wavelet Denoising Functions from the provided code ---
...
# --- Vector Field and Shape Creation Code (Copied from the provided code) ---
...
class VectorFieldComposer:
    def __init__(self):
        ...
        TILE_SIZE = 256  # Assuming TILE_SIZE is defined somewhere, if not define here.
    ...
    def apply_to_image(
        ...
        TILE_SIZE = image.shape[0]  # Assuming TILE_SIZE is image size
        self.pos_x, self.pos_y = np.meshgrid(np.arange(TILE_SIZE), np.arange(TILE_SIZE))
...
# --- Shape Creation Functions (Copied from the provided code) ---
import random
import math

TILE_SIZE = 256  # Define TILE_SIZE here if it's not globally defined.
...
# --- Dataset Class with Noise Extraction ---
from torch.utils.data import Dataset
from PIL import Image
import os

# Define these variables according to your setup
NUM_TILES = 10  # Example, replace with your actual number of tiles
TILE_IMAGE_PATHS = [
    f"../tiles/g78/00003.tif/tile_{i}.png" for i in range(1, NUM_TILES + 1)
]  # Example paths, adjust as needed
TILE_SIZE = 256  # Assuming TILE_SIZE is 256
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        denoised_image_uint8 = (denoised_image * 255).astype(
            np.uint8
        )  # Convert back to uint8 if needed for later use or visualization
        extracted_noise_uint8 = (extracted_noise * 255).astype(
            np.uint8
        )  # Convert noise to uint8 for visualization if needed
        ...
        return np.array(
            [
                image,
                denoised_image_uint8,
                extracted_noise_uint8,
                warped_image,
                applied_denoised_image,
            ]
        ).astype(np.float32), np.array([dU, dV]).astype(np.float32)
...
if __name__ == "__main__":
    ...
    num_rows = 10  # Number of example rows to show
    plt.figure(figsize=(12, 2 * num_rows))  # Adjust figure height for more rows

    for i in range(num_rows):
        sample_data, _ = dataset[i]

        original_image = sample_data[0]
        denoised_image = sample_data[1]
        extracted_noise = sample_data[2]
        morphed_image_denoised = sample_data[3]
        morphed_image = sample_data[4]

        # Calculate subplot index for each image in the row
        plot_index_original = i * 5 + 1
        plot_index_denoised = i * 5 + 2
        plot_index_noise = i * 5 + 3
        plot_index_morphed_denoised = i * 5 + 4
        plot_index_morphed = i * 5 + 5

        plt.subplot(num_rows, 5, plot_index_original)
        plt.imshow(original_image, cmap="gray")
        if i == 0:  # Only add title to the first row
            plt.title("Original Image")
        plt.axis("off")

        plt.subplot(num_rows, 5, plot_index_denoised)
        plt.imshow(denoised_image, cmap="gray")
        if i == 0:  # Only add title to the first row
            plt.title("Denoised Image")
        plt.axis("off")

        plt.subplot(num_rows, 5, plot_index_noise)
        plt.imshow(extracted_noise, cmap="gray")
        if i == 0:  # Only add title to the first row
            plt.title("Extracted Noise")
        plt.axis("off")

        plt.subplot(num_rows, 5, plot_index_morphed_denoised)
        plt.imshow(morphed_image_denoised, cmap="gray")
        if i == 0:  # Only add title to the first row
            plt.title("Morphed Denoised")
        plt.axis("off")

        plt.subplot(num_rows, 5, plot_index_morphed)
